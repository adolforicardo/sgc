...
export function Sidebar() {
  ...
  return (
    <aside className="w-56 min-h-screen bg-[#0d1117] text-white flex flex-col border-r border-slate-800/50">
      <div className="p-3 border-b border-slate-800/80">
        <Link href={ROUTES.DASHBOARD} className="flex items-center gap-3 group">
          <div className="w-8 h-8 rounded-md bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center font-bold text-xs">
            S
          </div>
          <div>
      ...
      {user && (
        <div className="p-2 border-t border-slate-800/80">
          <div className="px-2.5 py-1.5 rounded-md bg-slate-800/30">
            <p className="text-sm font-medium truncate text-slate-200">{user.name}</p>
            <p className="text-xs text-slate-500 truncate">{user.email}</p>
          </div>
          <button
            onClick={() => logout().then(() => { window.location.href = '/login'; })}
            className="w-full mt-1 text-left px-2.5 py-1.5 text-[11px] text-slate-500 hover:text-slate-300 hover:bg-slate-800/50 rounded-md transition-colors"
          >
            Terminar sessão
          </button>
        </div>
